...
CREATE TABLE `Motif`(
    `mot_num` varchar(20) not null,
    `mot_libelle` varchar(50),
...
CREATE TABLE `Connaitre` (
  ...
  KEY `FK_CONNAITRE_VISITEUR_RAPPORT_VISI` (`vis_matricule`, `rap_num`),
...
CREATE TABLE `RapportVisite` (
  ...
  `mot_num` varchar(20) NOT NULL DEFAULT '0',
  `rap_coef_confiance` double NOT NULL DEFAULT '0',
  `rap_date_saisie` DATE DEFAULT NULL,
  `rap_autre_motif` varchar(50) DEFAULT NULL,
  ...
  CONSTRAINT `FK_MOTIF_NUMMOTIF` FOREIGN KEY (`mot_num`) REFERENCES `Motif` (`mot_num`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
